...
public class TaskListController {
    ...
    @ApiResponses(
            value = {@ApiResponse(responseCode = "200", description = "Successfully retrieved list of task lists")})
    @GetMapping
    ...
    public ResponseEntity<TaskListDto> updateTaskList(
            @PathVariable("task_list_id") UUID taskListId, @Valid @RequestBody UpdateTaskListRequest request) {
        return ResponseEntity.status(HttpStatus.OK).body(taskListService.updateTaskList(taskListId, request));
    }

    @Operation(summary = "Delete a task list by ID")
    @ApiResponses(value = {@ApiResponse(responseCode = "204", description = "Task list deleted successfully")})
